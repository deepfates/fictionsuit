from .command_group import  CommandGroup, CommandFailure, CommandHandled, CommandNotFound, command_split
...
class Meta(CommandGroup):
    ...
    async def cmd_var(self, message: UserMessage, args: str) -> CommandFailure | CommandHandled:
        ...
        for group in self.command_groups:
            ...
            if type(result) is not CommandNotFound:
                if type(result) is CommandFailure:
                    return CommandFailure(f'Failed to store variable: Command "{cmd}" failed.\n{result.message}')
                if type(result) is CommandHandled:
                    return CommandFailure(f'Failed to store variable: Command "{cmd}" returns no value.')
